...
class FlyingCube {
  ...
  void render(float velocity, boolean shouldRotate) {
    ...
    if (emissive_prop > emissive_brightness) emissive_brightness = min(emissive_prop, 500); else emissive_brightness -= RAMPDOWN_FACTOR;
...
void doWarp() {
  if (bass > bass_avg) greaterThanFrames++; else greaterThanFrames = 0;
  if (level > level_avg * 1.5) loudFrames++; else loudFrames = 0;
